(ns bldgclean.actor
  "BldgCleanCoordActor -- the ISCO-08 7133 building structure cleaning
  crew scheduling/logistics coordination actor as a
  ...
  SCHEDULING/LOGISTICS ONLY -- it never performs building-cleaning
  work itself and never commits a proposal the BldgCleanCoordGovernor
  refuses. Modeled on cloud-itonami-isco-7121's roofcoord.actor.
  ...
  The unconditional invariant: the Building Cleaning Coordination
  Advisor can never directly commit a coordination record the
  BldgCleanCoordGovernor refuses -- every commit-record! call is
  gated behind `:decide`, and the governor's scope-exclusion rule
  makes any proposal to finalize a cleaning-execution decision
  (including a height-work go/no-go call) or override a site-safety
  officer's judgment an unconditional, permanent :hold."
...
(defn build-graph
  "Build a compiled BldgCleanCoordActor graph. `store` implements
  `bldgclean.store/Store`. `advisor` implements
  `bldgclean.advisor/Advisor` (defaults to `mock-advisor`).
...
(defn approve!
  ...
  can never be used to approve a cleaning-execution finalization
  (including a height-work go/no-go call) or a site-safety-officer
  judgment override."
  [graph thread-id]
