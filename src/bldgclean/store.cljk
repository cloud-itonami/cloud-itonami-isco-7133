(ns bldgclean.store
  "SSoT for the ISCO-08 7133 building structure cleaning crew
  ...
  and cleaning-materials/equipment-order coordination under this
  advisor/governor pair, which never dispatches hardware itself, never
  performs building-cleaning work, and never finalizes a
  cleaning-execution decision (including a height-work go/no-go call)
  or overrides a site-safety officer's judgment). Modeled on
  cloud-itonami-isco-7121's roofcoord.store.

  Domain:

    site    -- a registered building-cleaning job site (:site-id,
               :name, :address). Window washing and building-exterior
               / facade cleaning happen at height on active job
               sites, and cleaning agents used on-site carry a
               chemical-exposure risk alongside the fall-from-height
               risk.
    ...
    record  -- a committed operating record (a logged work record,
               ...
               commit-record!. This actor coordinates job-site
               scheduling/logistics ONLY -- a `record` is a
               coordination artifact, never a cleaning-execution act
               (including a height-work go/no-go decision) or a
