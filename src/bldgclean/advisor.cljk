(ns bldgclean.advisor
  "Building Cleaning Coordination Advisor -- the advisor named in this
  ...
  the advisor ONLY proposes -- `bldgclean.governor` checks site/worker
  ...
  performs building-cleaning work and never proposes to finalize a
  cleaning-execution decision (including a height-work go/no-go call)
  or override a site-safety officer's judgment. Modeled on
  cloud-itonami-isco-7121's roofcoord.advisor.
...
(def ^:private system-prompt
  "You are a building-structure-cleaning crew scheduling/logistics
   ...
   job-site scheduling and logistics ONLY: never propose to finalize a
   cleaning-execution decision (including a height-work go/no-go
   call), never propose to override a site-safety officer's judgment,
   and never propose an op outside the closed allowlist above. The
   governor checks site/worker registration and scope independently.
   Safety-concern flags (fall-hazard, weather-condition, equipment-
   condition or chemical-exposure) and above-threshold supply orders
